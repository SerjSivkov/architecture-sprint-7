...
# Функция для создания привязок ролей разработчиков/devops к ролям в неймспейсе
bind_developer_role() {
  local namespace=$1
  local group=$2
...
# Функция для создания привязок ролей операционной команды к ролям в неймспейсе
bind_viewer_role() {
  local namespace=$1
  local group=$2
